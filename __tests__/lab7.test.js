describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    */
      ...
      for(let i = 1; i < prodItemsData.length; i++){
        ...
        if(temp.title.length == 0) { allArePopulated = false;}
        if(temp.price.length == 0) { allArePopulated = false;}
        if(temp.image.length == 0) { allArePopulated = false;}
      }
      expect(allArePopulated).toBe(true);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cartValue = await cartCount.getProperty('innerText');
    const cartText = await cartValue.jsonValue();
